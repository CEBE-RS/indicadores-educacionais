  ...
  main.innerHTML = `
    ...
    <div class="charts-grid" style="display:grid;grid-template-columns:1fr;gap:10px">
      <div class="chart-card d2">
        ...
        <div class="chart-source">${FONTE_REND} · Recorte por série disponível de 2012 em diante (exceto 2015), via arquivos oficiais por município do INEP.</div>
      </div>
    </div>
    ...
    if (host) {
      host.innerHTML = `
        <div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:var(--text-sec);gap:6px;padding:20px">
          ...
          <div style="font-size:11.5px;max-width:420px;line-height:1.5">As taxas por série (1º–9º ano e 1ª–4ª série EM) estão disponíveis de <strong>2012 em diante (exceto 2015)</strong>, conforme os arquivos oficiais por município do INEP. Selecione um desses anos para visualizar.</div>
        </div>`;
    }
    return;
  }
